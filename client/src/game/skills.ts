import type {
  ActiveSkillDefinition,
  ClassId,
  SkillDefinition,
} from "./types";
import { getOverchargeRankValues } from "./skillOvercharge";
import { RUNECASTER_SKILL_RUNE_SEQUENCES } from "./runecasterRunes";

const BEGINNER_SKILL_COOLDOWN_MS = 10000;
const BEGINNER_BUFF_DURATION_MS = 90000;
const FIRST_CLASS_LONG_BUFF_DURATION_MS = 300000;
const FIRST_CLASS_LONG_BUFF_COOLDOWN_MS = 298000;
const FIRST_CLASS_BUFF_REFRESH_WINDOW_MS = 2000;
const OVERCHARGE_BUFF_DURATION_MS = 120000;
const OVERCHARGE_BUFF_COOLDOWN_MS = 118000;
export const DEFAULT_SKILL_COOLDOWN_MS = 10000;

export const SKILL_DEFINITIONS: Record<SkillDefinition["id"], SkillDefinition> = {
  throw_rock: {
    id: "throw_rock",
    classId: "beginner",
    displayName: "Throw Rock",
    tags: ["Offensive", "Single Target", "Taunt"],
    type: "active",
    range: 4,
    cooldownMs: BEGINNER_SKILL_COOLDOWN_MS,
    effect: { type: "taunt", durationMs: 3000 },
  },
  kick: {
    id: "kick",
    classId: "beginner",
    displayName: "Kick",
    tags: ["Offensive", "Damage", "Single Target"],
    type: "active",
    range: 5,
    cooldownMs: BEGINNER_SKILL_COOLDOWN_MS,
    effect: {
      type: "lungeDamage",
      damageType: "physical",
      powerMultiplier: 1,
      lungeDistance: 5,
    },
  },
  guard_up: {
    id: "guard_up",
    classId: "beginner",
    displayName: "Guard Up",
    tags: ["Defensive", "Shield", "Safety"],
    type: "active",
    range: 0,
    cooldownMs: BEGINNER_SKILL_COOLDOWN_MS,
    effect: {
      type: "shieldBlock",
      durationMs: 6000,
      blocks: 1,
      blockedDamageTypes: ["physical"],
    },
  },
  first_aid: {
    id: "first_aid",
    classId: "beginner",
    displayName: "First Aid",
    tags: ["Heal", "Safety"],
    type: "active",
    range: 3,
    cooldownMs: BEGINNER_SKILL_COOLDOWN_MS,
    effect: { type: "heal", powerMultiplier: 5 },
  },
  deep_breath: {
    id: "deep_breath",
    classId: "beginner",
    displayName: "Deep Breath",
    tags: ["Self Buff", "Safety"],
    type: "active",
    range: 0,
    cooldownMs: BEGINNER_SKILL_COOLDOWN_MS,
    effect: {
      type: "selfBuff",
      bonusDamage: 1,
      durationMs: BEGINNER_BUFF_DURATION_MS,
      hpCost: 0,
    },
  },
  rally_call: {
    id: "rally_call",
    classId: "beginner",
    displayName: "Rally Call",
    tags: ["Buff", "Safety"],
    type: "active",
    range: 4,
    cooldownMs: BEGINNER_SKILL_COOLDOWN_MS,
    effect: {
      type: "allyBuff",
      bonusDamage: 1,
      durationMs: BEGINNER_BUFF_DURATION_MS,
    },
  },
  field_hands: {
    id: "field_hands",
    classId: "beginner",
    displayName: "Field Hands",
    tags: ["Gathering", "Resource Buff", "Tool Buff"],
    type: "active",
    range: 0,
    cooldownMs: BEGINNER_SKILL_COOLDOWN_MS,
    effect: {
      type: "gatherBuff",
      bonusGatherSpeed: 1,
      durationMs: BEGINNER_BUFF_DURATION_MS,
    },
  },
  follow_through: {
    id: "follow_through",
    classId: "beginner",
    displayName: "Follow Through",
    tags: ["Offensive", "Damage", "Single Target"],
    type: "active",
    range: 1,
    cooldownMs: BEGINNER_SKILL_COOLDOWN_MS,
    effect: {
      type: "followThrough",
      damageType: "physical",
      powerMultiplier: 1,
      conditionalBonusMultiplier: 0.2,
    },
  },
  resourcefulness: {
    id: "resourcefulness",
    classId: "beginner",
    displayName: "Resourcefulness",
    tags: ["Heal", "Safety"],
    type: "passive",
    range: 0,
    effect: { type: "resourcefulness" },
  },
  steady_nerves: {
    id: "steady_nerves",
    classId: "beginner",
    displayName: "Steady Nerves",
    tags: ["Defensive", "Safety", "Control"],
    type: "passive",
    range: 0,
    effect: { type: "steadyNerves" },
  },
  duelist_challenge: {
    id: "duelist_challenge",
    classId: "blade",
    displayName: "Duelist Challenge",
    tags: ["Offensive", "Single Target", "Taunt"],
    type: "active",
    range: 4,
    cooldownMs: 12000,
    effect: { type: "taunt", durationMs: 4000 },
  },
  second_wind: {
    id: "second_wind",
    classId: "blade",
    displayName: "Second Wind",
    tags: ["Heal", "Self Healing", "Safety"],
    type: "active",
    range: 0,
    cooldownMs: 30000,
    effect: { type: "selfPercentHeal", healPercent: 20 },
  },
  blade_parry: {
    id: "blade_parry",
    classId: "blade",
    displayName: "Blade Parry",
    tags: ["Defensive", "Safety", "Damage Mitigation"],
    type: "active",
    range: 0,
    cooldownMs: 15000,
    effect: {
      type: "damageMitigation",
      durationMs: 20000,
      mitigationPercent: 50,
      procs: 2,
      mitigatedDamageTypes: ["physical"],
    },
  },
  edge_focus: {
    id: "edge_focus",
    classId: "blade",
    displayName: "Edge Focus",
    tags: ["Offensive", "Self Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "selfBuff",
      bonusDamage: 1,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      hpCost: 0,
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  press_the_opening: {
    id: "press_the_opening",
    classId: "blade",
    displayName: "Press the Opening",
    tags: ["Offensive", "Party Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "partyClassBuff",
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      physicalDamageBonusPercent: 5,
      primaryStatBonusPercentByStat: { strength: 5 },
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  woodcutter_rhythm: {
    id: "woodcutter_rhythm",
    classId: "blade",
    displayName: "Woodcutter Rhythm",
    tags: ["Gathering", "Resource Buff", "Tool Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "gatherBuff",
      bonusGatherSpeed: 2,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      resourceType: "wood",
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  flash_step: {
    id: "flash_step",
    classId: "blade",
    displayName: "Flash Step",
    tags: ["Mobility", "Dash", "Escape"],
    type: "active",
    range: 0,
    cooldownMs: 20000,
    effect: { type: "quickStep", distance: 2 },
  },
  sweeping_strike: {
    id: "sweeping_strike",
    classId: "blade",
    displayName: "Sweeping Strike",
    tags: ["Offensive", "Damage", "Single Target", "AoE"],
    type: "active",
    range: 1,
    cooldownMs: 10000,
    effect: {
      type: "sweepingDamage",
      damageType: "physical",
      mainPowerMultiplier: 1.5,
      splashPowerMultiplier: 1,
      splashRange: 1.5,
    },
  },
  duelists_momentum: {
    id: "duelists_momentum",
    classId: "blade",
    displayName: "Duelist's Momentum",
    tags: ["Offensive", "Damage"],
    type: "passive",
    range: 0,
    effect: { type: "duelistsMomentum" },
  },
  riposte_training: {
    id: "riposte_training",
    classId: "blade",
    displayName: "Riposte Training",
    tags: ["Offensive", "Damage", "Defensive"],
    type: "passive",
    range: 0,
    effect: { type: "riposteTraining" },
  },
  shield_challenge: {
    id: "shield_challenge",
    classId: "aegis",
    displayName: "Shield Challenge",
    tags: ["Defensive", "Taunt", "Multi Target"],
    type: "active",
    range: 4,
    cooldownMs: 12000,
    effect: { type: "multiTaunt", maxTargets: 2, durationMs: 6000 },
  },
  hold_fast: {
    id: "hold_fast",
    classId: "aegis",
    displayName: "Hold Fast",
    tags: ["Defensive", "Shield", "Safety", "Damage Mitigation", "Self Buff"],
    type: "active",
    range: 0,
    cooldownMs: 30000,
    effect: {
      type: "holdFast",
      defenseBonusPercent: 25,
      defenseDurationMs: 10000,
      absorbPercentMaxHealth: 10,
      absorbDurationMs: 5000,
      immobilizeDurationMs: 5000,
    },
  },
  guard_wall: {
    id: "guard_wall",
    classId: "aegis",
    displayName: "Guard Wall",
    tags: ["Defensive", "Shield", "Safety"],
    type: "active",
    range: 0,
    cooldownMs: 15000,
    effect: {
      type: "absorbShield",
      durationMs: 10000,
      absorbPercentMaxHealth: 15,
    },
  },
  iron_stance: {
    id: "iron_stance",
    classId: "aegis",
    displayName: "Iron Stance",
    tags: ["Defensive", "Self Buff", "Safety", "Damage Mitigation"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "selfMitigationBuff",
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      mitigationPercent: 10,
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  shield_formation: {
    id: "shield_formation",
    classId: "aegis",
    displayName: "Shield Formation",
    tags: ["Defensive", "Party Buff", "Shield", "Safety", "Damage Mitigation"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "partyClassBuff",
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      mitigationPercent: 8,
      primaryStatBonusPercentByStat: { constitution: 5 },
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  stonebreaker_rhythm: {
    id: "stonebreaker_rhythm",
    classId: "aegis",
    displayName: "Stonebreaker Rhythm",
    tags: ["Gathering", "Resource Buff", "Tool Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "gatherBuff",
      bonusGatherSpeed: 2,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      resourceType: "ore",
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  shield_rush: {
    id: "shield_rush",
    classId: "aegis",
    displayName: "Shield Rush",
    tags: ["Mobility", "Dash", "Escape"],
    type: "active",
    range: 2,
    cooldownMs: 20000,
    effect: { type: "quickStep", distance: 2 },
  },
  shield_shockwave: {
    id: "shield_shockwave",
    classId: "aegis",
    displayName: "Shield Shockwave",
    tags: ["Defensive", "Control", "AoE", "Taunt", "Damage"],
    type: "active",
    range: 0,
    cooldownMs: 15000,
    effect: {
      type: "shockwave",
      damageType: "physical",
      powerMultiplier: 0.7,
      radius: 2,
      bindDurationMs: 1000,
      tauntDurationMs: 5000,
    },
  },
  rooted_bastion: {
    id: "rooted_bastion",
    classId: "aegis",
    displayName: "Rooted Bastion",
    tags: ["Defensive", "Safety"],
    type: "passive",
    range: 0,
    effect: { type: "rootedBastion" },
  },
  unbroken_line: {
    id: "unbroken_line",
    classId: "aegis",
    displayName: "Unbroken Line",
    tags: ["Defensive", "Taunt", "Safety"],
    type: "passive",
    range: 0,
    effect: { type: "unbrokenLine" },
  },
  pinning_shot: {
    id: "pinning_shot",
    classId: "hunter",
    displayName: "Pinning Shot",
    tags: ["Control", "Single Target", "Safety"],
    type: "active",
    range: 4,
    cooldownMs: 12000,
    effect: { type: "pinningShot", durationMs: 3000 },
  },
  fake_death: {
    id: "fake_death",
    classId: "hunter",
    displayName: "Fake Death",
    tags: ["Defensive", "Safety", "Escape", "Self Buff"],
    type: "active",
    range: 0,
    cooldownMs: 30000,
    effect: {
      type: "fakeDeath",
      fakeDeathDurationMs: 3000,
      nextAttackDamageMultiplierBonus: 0.3,
      nextAttackBonusDurationMs: 10000,
    },
  },
  evasive_instinct: {
    id: "evasive_instinct",
    classId: "hunter",
    displayName: "Evasive Instinct",
    tags: ["Defensive", "Safety", "Damage Mitigation", "Self Buff"],
    type: "active",
    range: 0,
    cooldownMs: 15000,
    effect: { type: "forcedEvasion", durationMs: 10000 },
  },
  hunters_focus: {
    id: "hunters_focus",
    classId: "hunter",
    displayName: "Hunter's Focus",
    tags: ["Offensive", "Self Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "selfBuff",
      bonusDamage: 1,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      hpCost: 0,
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  poison_coating: {
    id: "poison_coating",
    classId: "hunter",
    displayName: "Poison Coating",
    tags: ["Offensive", "Party Buff", "DoT"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "partyClassBuff",
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      primaryStatBonusPercentByStat: { dexterity: 5 },
      poisonCoating: {
        poisonDurationMs: 4000,
        poisonTickIntervalMs: 2000,
        poisonDamageAttackPowerPercent: 1,
        sourceKey: "poison_coating",
      },
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  herbalist_rhythm: {
    id: "herbalist_rhythm",
    classId: "hunter",
    displayName: "Herbalist Rhythm",
    tags: ["Gathering", "Resource Buff", "Tool Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "gatherBuff",
      bonusGatherSpeed: 2,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      resourceType: "herb",
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  skirmish_shot: {
    id: "skirmish_shot",
    classId: "hunter",
    displayName: "Skirmish Shot",
    tags: ["Mobility", "Dash", "Escape", "Damage", "Single Target"],
    type: "active",
    range: 4,
    cooldownMs: 20000,
    effect: {
      type: "skirmishShot",
      distance: 2,
      damageType: "physical",
      powerMultiplier: 1.4,
    },
  },
  arrow_burst: {
    id: "arrow_burst",
    classId: "hunter",
    displayName: "Arrow Burst",
    tags: ["Offensive", "Damage", "Multi Target", "AoE"],
    type: "active",
    range: 4,
    cooldownMs: 15000,
    effect: {
      type: "arrowBurst",
      damageType: "physical",
      powerMultiplier: 1.2,
      radius: 3,
    },
  },
  headhunter: {
    id: "headhunter",
    classId: "hunter",
    displayName: "Headhunter",
    tags: ["Offensive", "Damage"],
    type: "passive",
    range: 0,
    effect: { type: "headhunter" },
  },
  exploit_the_snare: {
    id: "exploit_the_snare",
    classId: "hunter",
    displayName: "Exploit the Snare",
    tags: ["Offensive", "Damage", "Control"],
    type: "passive",
    range: 0,
    effect: { type: "exploitTheSnare" },
  },
  threatening_roar: {
    id: "threatening_roar",
    classId: "beast",
    displayName: "Threatening Roar",
    tags: ["Defensive", "Taunt", "Control", "AoE"],
    type: "active",
    range: 3,
    cooldownMs: 15000,
    effect: { type: "multiTaunt", maxTargets: 2, durationMs: 4000 },
  },
  blood_feast: {
    id: "blood_feast",
    classId: "beast",
    displayName: "Blood Feast",
    tags: ["Heal", "Self Healing", "Safety", "Offensive", "Self Buff"],
    type: "active",
    range: 0,
    cooldownMs: 30000,
    effect: {
      type: "lifestealBuff",
      durationMs: 10000,
      lifestealPercent: 10,
    },
  },
  rugged_hide: {
    id: "rugged_hide",
    classId: "beast",
    displayName: "Rugged Hide",
    tags: ["Defensive", "Safety", "Damage Mitigation", "Self Buff"],
    type: "active",
    range: 0,
    cooldownMs: 20000,
    effect: {
      type: "selfMitigationBuff",
      durationMs: 10000,
      mitigationPercent: 20,
    },
  },
  feral_surge: {
    id: "feral_surge",
    classId: "beast",
    displayName: "Feral Surge",
    tags: ["Offensive", "Self Buff", "Mobility"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "selfBuff",
      bonusDamage: 1,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      hpCost: 0,
      movementSpeedBonusPercent: 20,
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  pack_frenzy: {
    id: "pack_frenzy",
    classId: "beast",
    displayName: "Pack Frenzy",
    tags: ["Offensive", "Party Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "partyClassBuff",
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      primaryStatBonusPercentByStat: { strength: 5, dexterity: 5 },
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  stoneclaw_rhythm: {
    id: "stoneclaw_rhythm",
    classId: "beast",
    displayName: "Stoneclaw Rhythm",
    tags: ["Gathering", "Resource Buff", "Tool Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "gatherBuff",
      bonusGatherSpeed: 2,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      resourceType: "ore",
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  pounce: {
    id: "pounce",
    classId: "beast",
    displayName: "Pounce",
    tags: ["Mobility", "Dash", "Escape", "Damage", "Single Target"],
    type: "active",
    range: 4,
    cooldownMs: 20000,
    effect: {
      type: "pounce",
      distance: 2,
      damageType: "physical",
      powerMultiplier: 1.45,
    },
  },
  maul_sweep: {
    id: "maul_sweep",
    classId: "beast",
    displayName: "Maul Sweep",
    tags: ["Offensive", "Damage", "AoE", "Multi Target", "Control"],
    type: "active",
    range: 0,
    cooldownMs: 15000,
    effect: {
      type: "maulSweep",
      damageType: "physical",
      powerMultiplier: 1.15,
      radius: 2,
      disarmDurationMs: 1500,
    },
  },
  blood_scent: {
    id: "blood_scent",
    classId: "beast",
    displayName: "Blood Scent",
    tags: ["Offensive", "Damage"],
    type: "passive",
    range: 0,
    effect: { type: "bloodScent" },
  },
  pack_instinct: {
    id: "pack_instinct",
    classId: "beast",
    displayName: "Pack Instinct",
    tags: ["Offensive", "Damage"],
    type: "passive",
    range: 0,
    effect: { type: "packInstinct" },
  },
  elemental_bolt: {
    id: "elemental_bolt",
    classId: "elementalist",
    displayName: "Elemental Bolt",
    tags: ["Offensive", "Damage", "Single Target"],
    type: "active",
    offensiveSpell: true,
    range: 5,
    cooldownMs: 12000,
    effect: { type: "damage", damageType: "magic", powerMultiplier: 1.5 },
  },
  mana_shield: {
    id: "mana_shield",
    classId: "elementalist",
    displayName: "Mana Shield",
    tags: ["Defensive", "Shield", "Safety", "Maintenance"],
    type: "active",
    range: 0,
    cooldownMs: 30000,
    effect: { type: "manaShield", absorbPercentMaxHealth: 15 },
  },
  frost_armor: {
    id: "frost_armor",
    classId: "elementalist",
    displayName: "Frost Armor",
    tags: ["Defensive", "Buff", "Safety", "Damage Mitigation"],
    type: "active",
    range: 5,
    cooldownMs: 6000,
    effect: {
      type: "frostArmor",
      durationMs: 60000,
      defenseBonusPercent: 10,
      mitigationPercent: 10,
    },
  },
  overcharge: {
    id: "overcharge",
    classId: "elementalist",
    displayName: "Overcharge",
    tags: ["Offensive", "Self Buff", "Maintenance"],
    type: "active",
    range: 0,
    cooldownMs: OVERCHARGE_BUFF_COOLDOWN_MS,
    effect: {
      type: "overcharge",
      durationMs: OVERCHARGE_BUFF_DURATION_MS,
      ...getOverchargeRankValues(1),
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  arcane_conduit: {
    id: "arcane_conduit",
    classId: "elementalist",
    displayName: "Arcane Conduit",
    tags: ["Offensive", "Party Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "partyClassBuff",
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      magicDamageBonusPercent: 5,
      primaryStatBonusPercentByStat: { intelligence: 5 },
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  emberwood_rhythm: {
    id: "emberwood_rhythm",
    classId: "elementalist",
    displayName: "Emberwood Rhythm",
    tags: ["Gathering", "Resource Buff", "Tool Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "gatherBuff",
      bonusGatherSpeed: 2,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      resourceType: "wood",
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  flame_step: {
    id: "flame_step",
    classId: "elementalist",
    displayName: "Flame Step",
    tags: ["Mobility", "Dash", "Escape", "Damage", "DoT", "Single Target"],
    type: "active",
    offensiveSpell: true,
    range: 5,
    cooldownMs: 20000,
    effect: {
      type: "flameStep",
      distance: 2,
      burnDurationMs: 4000,
      burnTickIntervalMs: 2000,
      burnDamageMagicPowerPercent: 4,
      sourceKey: "flame_step",
    },
  },
  fire_burst: {
    id: "fire_burst",
    classId: "elementalist",
    displayName: "FireBurst",
    tags: ["Offensive", "Damage", "Multi Target", "AoE", "DoT"],
    type: "active",
    offensiveSpell: true,
    range: 5,
    cooldownMs: 15000,
    effect: {
      type: "fireBurst",
      damageType: "magic",
      powerMultiplier: 1.15,
      radius: 3,
      burnDurationMs: 4000,
      burnTickIntervalMs: 2000,
      burnDamageMagicPowerPercent: 2,
      sourceKey: "fire_burst",
    },
  },
  stable_overcharge: {
    id: "stable_overcharge",
    classId: "elementalist",
    displayName: "Stable Overcharge",
    tags: ["Offensive", "Self Buff"],
    type: "passive",
    range: 0,
    effect: { type: "stableOvercharge" },
  },
  arcane_crescendo: {
    id: "arcane_crescendo",
    classId: "elementalist",
    displayName: "Arcane Crescendo",
    tags: ["Offensive", "Damage", "AoE"],
    type: "passive",
    range: 0,
    effect: { type: "arcaneCrescendo" },
  },
  binding_rune: {
    id: "binding_rune",
    classId: "runecaster",
    displayName: "Binding Rune",
    tags: ["Control", "Trap", "Single Target", "Safety"],
    type: "active",
    range: 5,
    cooldownMs: 12000,
    runeWords: RUNECASTER_SKILL_RUNE_SEQUENCES.binding_rune,
    effect: { type: "pinningShot", durationMs: 3500 },
  },
  rune_lance: {
    id: "rune_lance",
    classId: "runecaster",
    displayName: "Rune Lance",
    tags: ["Offensive", "Damage", "Single Target"],
    type: "active",
    range: 5,
    cooldownMs: 12000,
    runeWords: RUNECASTER_SKILL_RUNE_SEQUENCES.rune_lance,
    effect: { type: "damage", damageType: "magic", powerMultiplier: 1.4 },
  },
  warding_glyph: {
    id: "warding_glyph",
    classId: "runecaster",
    displayName: "Warding Glyph",
    tags: ["Defensive", "Barrier", "Safety", "Damage Mitigation", "Buff"],
    type: "active",
    range: 5,
    cooldownMs: 15000,
    runeWords: RUNECASTER_SKILL_RUNE_SEQUENCES.warding_glyph,
    effect: {
      type: "barrierBlock",
      durationMs: 10000,
      blocks: 2,
      blockedDamageTypes: ["physical", "magic"],
    },
  },
  rewind_rune: {
    id: "rewind_rune",
    classId: "runecaster",
    displayName: "Rewind Rune",
    tags: ["Heal", "Defensive", "Buff", "Safety"],
    type: "active",
    range: 5,
    cooldownMs: 10000,
    runeWords: RUNECASTER_SKILL_RUNE_SEQUENCES.rewind_rune,
    effect: {
      type: "rewindRune",
      durationMs: 20000,
      healPercentRecordedDamage: 35,
      tickIntervalMs: 1000,
    },
  },
  runic_focus: {
    id: "runic_focus",
    classId: "runecaster",
    displayName: "Runic Focus",
    tags: ["Self Buff", "Maintenance"],
    type: "active",
    range: 0,
    cooldownMs: 15000,
    runeWords: RUNECASTER_SKILL_RUNE_SEQUENCES.runic_focus,
    effect: { type: "runicFocus" },
  },
  leyline_matrix: {
    id: "leyline_matrix",
    classId: "runecaster",
    displayName: "Leyline Matrix",
    tags: ["Party Buff", "Safety", "Elemental Mitigation"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    runeWords: RUNECASTER_SKILL_RUNE_SEQUENCES.leyline_matrix,
    effect: {
      type: "partyClassBuff",
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      primaryStatBonusPercentByStat: { wisdom: 5 },
      mitigationPercent: 8,
      mitigatedDamageTypes: ["magic"],
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  stone_sigil_rhythm: {
    id: "stone_sigil_rhythm",
    classId: "runecaster",
    displayName: "Stone Sigil Rhythm",
    tags: ["Gathering", "Resource Buff", "Tool Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    runeWords: RUNECASTER_SKILL_RUNE_SEQUENCES.stone_sigil_rhythm,
    effect: {
      type: "gatherBuff",
      bonusGatherSpeed: 2,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      resourceType: "ore",
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  rune_step: {
    id: "rune_step",
    classId: "runecaster",
    displayName: "Rune Step",
    tags: ["Mobility", "Dash", "Escape", "Control", "Trap", "Safety"],
    type: "active",
    range: 5,
    cooldownMs: 20000,
    runeWords: RUNECASTER_SKILL_RUNE_SEQUENCES.rune_step,
    effect: {
      type: "runeStep",
      distance: 2,
      trapRadius: 2,
      trapImmobilizeDurationMs: 2000,
    },
  },
  word_resonance: {
    id: "word_resonance",
    classId: "runecaster",
    displayName: "Word Resonance",
    tags: ["Offensive", "Buff"],
    type: "passive",
    range: 0,
    effect: { type: "wordResonance" },
  },
  living_inscription: {
    id: "living_inscription",
    classId: "runecaster",
    displayName: "Living Inscription",
    tags: ["Offensive", "Buff"],
    type: "passive",
    range: 0,
    effect: { type: "livingInscription" },
  },
  blinding_ray: {
    id: "blinding_ray",
    classId: "lightbearer",
    displayName: "Blinding Ray",
    tags: ["Control", "Single Target", "Safety"],
    type: "active",
    range: 5,
    cooldownMs: 12000,
    effect: { type: "silencingRay", durationMs: 3000 },
  },
  light_mend: {
    id: "light_mend",
    classId: "lightbearer",
    displayName: "Light Mend",
    tags: ["Heal", "Safety"],
    type: "active",
    range: 5,
    cooldownMs: 20000,
    effect: { type: "heal", powerMultiplier: 2 },
  },
  sanctuary_veil: {
    id: "sanctuary_veil",
    classId: "lightbearer",
    displayName: "Sanctuary Veil",
    tags: ["Defensive", "Barrier", "Safety", "Buff"],
    type: "active",
    range: 5,
    cooldownMs: 15000,
    effect: {
      type: "barrierBlock",
      durationMs: 6000,
      blocks: 1,
      blockedDamageTypes: ["physical", "magic"],
      healPercentMaxHealthOnConsume: 5,
    },
  },
  guiding_light: {
    id: "guiding_light",
    classId: "lightbearer",
    displayName: "Guiding Light",
    tags: ["Heal", "Buff", "Safety"],
    type: "active",
    range: 5,
    cooldownMs: 6000,
    effect: {
      type: "healOverTime",
      durationMs: 15000,
      tickIntervalMs: 2000,
      healPercentMaxHealth: 1,
    },
  },
  radiant_benediction: {
    id: "radiant_benediction",
    classId: "lightbearer",
    displayName: "Radiant Benediction",
    tags: ["Party Buff", "Safety"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "partyClassBuff",
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      primaryStatBonusPercentByStat: { wisdom: 5 },
      healingReceivedBonusPercent: 5,
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  herbalist_hymn: {
    id: "herbalist_hymn",
    classId: "lightbearer",
    displayName: "Herbalist Hymn",
    tags: ["Gathering", "Resource Buff", "Tool Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "gatherBuff",
      bonusGatherSpeed: 2,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      resourceType: "herb",
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  dawn_step: {
    id: "dawn_step",
    classId: "lightbearer",
    displayName: "Dawn Step",
    tags: ["Mobility", "Dash", "Escape", "Control", "Safety"],
    type: "active",
    range: 5,
    cooldownMs: 20000,
    effect: {
      type: "dawnStep",
      distance: 2,
      disarmRadius: 2,
      disarmDurationMs: 1000,
    },
  },
  circle_of_renewal: {
    id: "circle_of_renewal",
    classId: "lightbearer",
    displayName: "Circle of Renewal",
    tags: ["Heal", "AoE", "Multi Target", "Safety"],
    type: "active",
    range: 5,
    cooldownMs: 15000,
    effect: {
      type: "circleOfRenewal",
      powerMultiplier: 1,
      radius: 5,
    },
  },
  overflowing_grace: {
    id: "overflowing_grace",
    classId: "lightbearer",
    displayName: "Overflowing Grace",
    tags: ["Heal", "Barrier", "Safety"],
    type: "passive",
    range: 0,
    effect: { type: "overflowingGrace" },
  },
  many_beacons: {
    id: "many_beacons",
    classId: "lightbearer",
    displayName: "Many Beacons",
    tags: ["Heal", "Buff", "Safety"],
    type: "passive",
    range: 0,
    effect: { type: "manyBeacons" },
  },
  whip_prison: {
    id: "whip_prison",
    classId: "penitent",
    displayName: "Whip Prison",
    tags: ["Offensive", "Defensive", "Control", "Single Target", "Safety", "DoT"],
    type: "active",
    range: 3,
    cooldownMs: 12000,
    effect: {
      type: "whipPrison",
      controlDurationMs: 3000,
      bleedDurationMs: 3000,
      bleedTickIntervalMs: 1000,
      bleedDamageAttackPowerPercent: 3,
      sourceKey: "whip_prison",
    },
  },
  flagellant_lash: {
    id: "flagellant_lash",
    classId: "penitent",
    displayName: "Flagellant Lash",
    tags: ["Offensive", "Damage", "Single Target", "DoT", "Self Cost - HP"],
    type: "active",
    range: 3,
    cooldownMs: 12000,
    effect: {
      type: "flagellantLash",
      damageType: "physical",
      powerMultiplier: 1.45,
      hpCostCurrentPercent: 3,
      bleedDurationMs: 4000,
      bleedTickIntervalMs: 1000,
      bleedDamageAttackPowerPercent: 2,
      sourceKey: "flagellant_lash",
    },
  },
  martyrs_veil: {
    id: "martyrs_veil",
    classId: "penitent",
    displayName: "Martyr's Veil",
    tags: ["Defensive", "Barrier", "Safety", "Buff", "Self Cost - HP"],
    type: "active",
    range: 5,
    cooldownMs: 15000,
    effect: {
      type: "sacrificialBarrier",
      durationMs: 10000,
      blocks: 2,
      blockedDamageTypes: ["physical", "magic"],
      hpCostCurrentPercent: 3,
    },
  },
  penitents_gift: {
    id: "penitents_gift",
    classId: "penitent",
    displayName: "Penitent's Gift",
    tags: ["Heal", "Self Healing", "Safety", "Self Cost - HP"],
    type: "active",
    range: 5,
    cooldownMs: 20000,
    effect: {
      type: "sacrificeHeal",
      hpCostCurrentPercent: 5,
      healSacrificeMultiplier: 3,
    },
  },
  eternal_hope: {
    id: "eternal_hope",
    classId: "penitent",
    displayName: "Eternal Hope",
    tags: [
      "Self Buff",
      "Safety",
      "Self Cost - HP",
      "Heal",
      "Self Healing",
      "Damage Mitigation",
    ],
    type: "active",
    range: 0,
    cooldownMs: 20000,
    effect: {
      type: "eternalHope",
      hpCostCurrentPercent: 10,
      durationMs: 25000,
      tickIntervalMs: 3000,
      healSacrificeMultiplier: 2,
      mitigationPercent: 20,
    },
  },
  burdened_benediction: {
    id: "burdened_benediction",
    classId: "penitent",
    displayName: "Burdened Benediction",
    tags: ["Party Buff", "Safety"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "partyClassBuff",
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      primaryStatBonusPercentByStat: { wisdom: 5, constitution: 5 },
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  woodcutting_penance: {
    id: "woodcutting_penance",
    classId: "penitent",
    displayName: "Woodcutting Penance",
    tags: ["Gathering", "Resource Buff", "Tool Buff"],
    type: "active",
    range: 0,
    cooldownMs: FIRST_CLASS_LONG_BUFF_COOLDOWN_MS,
    effect: {
      type: "gatherBuff",
      bonusGatherSpeed: 2,
      durationMs: FIRST_CLASS_LONG_BUFF_DURATION_MS,
      resourceType: "wood",
      refreshWindowMs: FIRST_CLASS_BUFF_REFRESH_WINDOW_MS,
    },
  },
  atonement_step: {
    id: "atonement_step",
    classId: "penitent",
    displayName: "Atonement Step",
    tags: [
      "Mobility",
      "Dash",
      "Escape",
      "Heal",
      "Control",
      "AoE",
      "Safety",
      "Self Cost - HP",
    ],
    type: "active",
    range: 4,
    cooldownMs: 20000,
    effect: {
      type: "atonementStep",
      distance: 2,
      hpCostCurrentPercent: 2,
      disarmRadius: 2,
      disarmDurationMs: 1000,
      healRadius: 2,
      healSacrificeMultiplier: 1,
    },
  },
  crimson_authority: {
    id: "crimson_authority",
    classId: "penitent",
    displayName: "Crimson Authority",
    tags: ["Offensive", "Control", "DoT"],
    type: "passive",
    range: 0,
    effect: { type: "crimsonAuthority" },
  },
  cruel_mercy: {
    id: "cruel_mercy",
    classId: "penitent",
    displayName: "Cruel Mercy",
    tags: ["Offensive", "Damage", "Heal", "Shield"],
    type: "passive",
    range: 0,
    effect: { type: "cruelMercy" },
  },
};

export function getSkillsForClass(classId: ClassId): SkillDefinition[] {
  return Object.values(SKILL_DEFINITIONS).filter(
    (skill) => skill.classId === classId,
  );
}

export function getActiveSkillsForClass(
  classId: ClassId,
): ActiveSkillDefinition[] {
  return getSkillsForClass(classId).filter(
    (skill): skill is ActiveSkillDefinition => skill.type === "active",
  );
}

export function getSkillCooldownMs(skill: SkillDefinition): number {
  return skill.cooldownMs ?? DEFAULT_SKILL_COOLDOWN_MS;
}
